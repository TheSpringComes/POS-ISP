...
from dataloader import create_dataloader_real
...
def run(
    ...
    with open(os.path.join(save_dir, "records.txt"), "w+") as f:
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            with dt[1]:
                ...
                retouch = torch.clamp(retouch, 0.0, 1.0)
                preds, train_out = model(retouch) if compute_loss else (model(retouch, augment=augment), None)
            ...
            for si, pred in enumerate(preds):
                ...
                '''
                predn.shape: torch.Size([84, 6])
                shapes[si][1]
                [[0.4275, 0.4266666666666667], [0.0, 85.0]]
                '''
    ...
    if save_json and len(jdict):
        ...
        anno_json = str(Path(f'../datasets/coco/annotations/instances_{task}2017.json'))
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=str(PROJECT_ROOT / 'pretrained' / 'yolov3.pt'), help='model path(s)')
