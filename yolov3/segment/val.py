...
def run(
    ...
    s = ('%22s' + '%11s' * 12) % ('Class', 'Images', 'Instances', 'Box(P', 'R', 'mAP50', 'mAP75', 'mAP50-95)', 'Mask(P', 'R',
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco-seg.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5m-seg.pt', help='model path(s)')
...
"""
python segment/val.py --weights yolov5m-seg.pt --project runs/val-seg --data data/coco-seg.yaml --name adaptive
"""
