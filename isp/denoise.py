...
def safe_sqrt(x):
    return torch.sqrt(torch.clamp_min(x, 0) + EPS)
    ...
    def forward(self, rgb, h):
        ...
        for x_shift in range(-self.r, self.r + 1):
            for y_shift in range(-self.r, self.r + 1):
                ...
                distance = safe_sqrt(torch.relu(self.box_sum((rgb - shifted_rgb) ** 2)))  # (N, 1, H, W)
    ...
    def forward(self, rgb, h):
        ...
        for x_shift in range(-self.r, self.r + 1):
            for y_shift in range(-self.r, self.r + 1):
                ...
                distance =  safe_sqrt(torch.relu(self.box_sum((y - shifted_y) ** 2)))  # (N, 1, H, W)
                weight = torch.exp(-distance / (torch.relu(h) + EPS))  # (N, 1, H, W)
...
class NonLocalMeansParam(nn.Module):
    ...
    def forward(self, rgb):
        ...
        distances =  safe_sqrt(torch.relu(dis))
        ...
        #         shifted_y = torch.roll(y, shifts=(y_shift, x_shift), dims=(2, 3))  # (N, 1, H, W)
        #
        #         # distance =  safe_sqrt(self.box_sum((y - shifted_y) ** 2) + EPS)  # (N, 1, H, W)
