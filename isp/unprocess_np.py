...
def adjust_random_brightness(image, s_range=(0.1, 0.3)):
    if isinstance(s_range, (list, tuple)):
        assert s_range[0] < s_range[1], "s_range[0] should less than s_range[1]"
...
def unprocess_wo_mosaic_and_wb(image, add_noise=False, brightness_range=None, noise_level=None, use_linear=False):
    """Unprocesses an image from sRGB to realistic raw data."""
    # Randomly creates image metadata.
    rgb2cam = random_ccm()
    cam2rgb = np.linalg.inv(rgb2cam)
    ...
    # Approximately inverts global tone mapping.
    image = inverse_smoothstep(image)
    # cv2.imwrite(f'unp_test_inverse_smoothstep.png',(image[..., ::-1]*255).astype(np.uint8))
    # Inverts gamma compression.
    image = gamma_expansion(image)

    # Inverts color correction.
    image = apply_ccm(image, rgb2cam)
    # cv2.imwrite(f'unp_test_ccm.png',(image[..., ::-1]*255).astype(np.uint8))

    # # Approximately inverts white balance and brightening.
    # image = safe_invert_gains(image, rgb_gain, red_gain, blue_gain)
    # cv2.imwrite(f'unp_test_wb.png',(image[..., ::-1]*255).astype(np.uint8))

    # Clips saturated pixels.
    image = np.clip(image, 0.0, 1.0)
    # cv2.imwrite(f'unp_test_gamma_expansion.png',(image[..., ::-1]*255).astype(np.uint8))
    # Applies a Bayer mosaic.
    # image = mosaic(image, pattern)
...
def unprocess_wo_mosaic_and_ccm(image, add_noise=False, brightness_range=None, noise_level=None, use_linear=False):
    """Unprocesses an image from sRGB to realistic raw data."""
    # Randomly creates image metadata.
    rgb2cam = random_ccm()
    cam2rgb = np.linalg.inv(rgb2cam)
    # rgb_gain, red_gain, blue_gain = random_gains()
    cv2.imwrite(f'unp_test_inp.png',(image[..., ::-1]*255).astype(np.uint8))
...
def unprocess_wo_mosaic_v2(image, add_noise=False, brightness_range=None, noise_level=None, use_linear=False):
    """Unprocesses an image from sRGB to realistic raw data."""
    # Randomly creates image metadata.
    rgb2cam = random_ccm()
    cam2rgb = np.linalg.inv(rgb2cam)
    ...
    image, _ = adjust_random_brightness(image, s_range=(0.5, 0.9))  # (0.6, 0.8)
    image = safe_invert_gains(image, rgb_gain, red_gain, blue_gain)
    image = gamma_expansion(image)
    image = inverse_smoothstep(image)
    image = apply_ccm(image, rgb2cam)
